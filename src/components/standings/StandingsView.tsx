...
function tzLabel(tz: string) {
  try {
    const parts = new Intl.DateTimeFormat('es-MX', { timeZone: tz, timeZoneName: 'long' }).formatToParts(new Date());
...
export default function StandingsView({ initialGroups, initialFixtures }: Props) {
  ...
  return (
    <div className={`${dark ? 'dark' : ''} min-h-screen font-display transition-colors duration-300 bg-[#f0f6f6] dark:bg-bg-1 text-gray-900 dark:text-white`}>
      {/* Top glow — subtle in light, vivid in dark */}
      <div
        className="pointer-events-none fixed inset-x-0 top-0 h-96 dark:opacity-30 opacity-10"
        style={{ background: 'radial-gradient(ellipse 80% 50% at 50% -10%, #f07820 0%, transparent 70%)' }}
      />

      <div className="relative z-10 mx-auto max-w-5xl px-4 pb-16 pt-8">
        {/* ── Top nav ── */}
        <div className="mb-8 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 text-xs text-gray-400 dark:text-white/40 transition hover:text-gray-600 dark:hover:text-white/70">
        ...
        <header className="mb-8 flex flex-col items-center gap-1 text-center">
          <p className="text-xs tracking-[0.3em] text-gray-400 dark:text-white/40 uppercase">FIFA World Cup 2026</p>
          <h1 className="text-3xl font-bold tracking-wider uppercase sm:text-4xl text-gray-900 dark:text-white">
            Tabla de Posiciones
          </h1>
          <div className="mt-2 flex flex-col items-center gap-1">
            <div className="flex items-center gap-3">
              <span className="text-xs text-gray-400 dark:text-white/30">
                Actualizado:{' '}
                {lastUpdated.toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit', second: '2-digit', timeZone: userTz })}
              </span>
              <button
                onClick={() => refresh(false)}
                disabled={refreshing}
                className="flex items-center gap-1 rounded px-2 py-0.5 text-xs text-gray-400 dark:text-white/40 transition hover:text-brand-orange disabled:opacity-50"
              >
                <RefreshIcon spinning={refreshing} />
                {refreshing ? 'Actualizando…' : 'Actualizar'}
              </button>
            </div>
            <span className="text-[10px] text-gray-400 dark:text-white/20 tracking-wide">
              ...
              {GROUP_LETTERS.map((letter) => {
                ...
                return (
                  <button
                    ...
                    className={[
                      'relative rounded-lg px-2.5 sm:px-3.5 py-2 text-sm font-semibold tracking-wider transition-all duration-200',
                      active
            ...
            <div className="mb-8 overflow-hidden rounded-2xl border border-gray-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.03] backdrop-blur-sm shadow-sm dark:shadow-none">
              ...
              <div className="flex items-center justify-between border-b border-gray-100 dark:border-white/[0.06] px-5 py-3.5">
                <h2 className="text-sm font-bold tracking-[0.2em] uppercase text-gray-700 dark:text-white/70">
                  Grupo {selectedGroup}
                  <span className="ml-2 text-gray-300 dark:text-white/30 font-normal">·</span>
              ...
              {currentGroup.entries.map((entry, idx) => {
                ...
                return (
                  <div
                    key={entry.team.id}
                    className={[
                      'group grid grid-cols-[2rem_1fr_2rem_2rem_2.5rem_2.5rem] sm:grid-cols-[2.5rem_1fr_repeat(7,2.5rem)_3rem] gap-x-1 px-4 sm:px-5 py-3.5 transition-colors duration-150',
                  ...
                  >
                    ...
                    <div className="flex items-center justify-center">
                      <span className={[
                        'flex h-6 w-6 sm:h-7 sm:w-7 items-center justify-center rounded-full text-xs font-bold',
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
                      ...
                      {isTop2 && (
                        <span className={[
                          'hidden shrink-0 rounded px-1.5 py-0.5 text-[9px] font-bold tracking-wider uppercase sm:block',
                          isFirst ? 'bg-brand-orange/15 text-brand-orange' : 'bg-brand-teal/15 text-brand-teal',
                        ].join(' ')}>
                    ...
                    <div className="flex items-center justify-center">
                      <span className={[
                        'flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-lg text-xs sm:text-sm font-bold',
...
function EmptyState({ text }: { text: string }) {
  return (
    <div className="rounded-xl border border-gray-200 dark:border-white/[0.05] p-6 text-center text-xs text-gray-400 dark:text-white/25">{text}</div>
  );
}
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="shrink-0 text-center">
          <span className="text-2xl font-bold text-gray-900 dark:text-white">
            {f.home.score ?? 0}
            <span className="mx-2 text-gray-300 dark:text-white/30">–</span>
            {f.away.score ?? 0}
          </span>
        </div>
...
function RefreshIcon({ spinning }: { spinning: boolean }) {
  return (
    <svg className={['h-3.5 w-3.5', spinning ? 'animate-spin' : ''].join(' ')} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}>
